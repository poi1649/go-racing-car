...
func TestGameRun(t *testing.T) {
	game, _ := racingcar.NewGame([]string{"test1", "test2"}, racingcar.NewFixedFuelGenerator([]int{1, 4}))
	result, _ := game.Run()
...
func TestGameGetWinnersNameWithSamePosition(t *testing.T) {
	game, _ := racingcar.NewGame([]string{"test1", "test2"}, racingcar.NewFixedFuelGenerator([]int{4, 4}))
	game.Run()
